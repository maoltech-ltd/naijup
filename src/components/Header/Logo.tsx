...
const Logo = ({user} : any) => {
  ...
  return ( 
    <Link href={link} className="flex items-center text-dark dark:text-light">
        ...
        {user.userId != '' ? (
          <Image src={user.profilePicture} alt={user.username} width={20} height={20} className="w-full h-auto rounded-full" quality={50} />
        ) : (
          <Image src={profileImg} alt="naijup" className="w-full h-auto rounded-full" quality={50} />
        )}
        </div>
